...
readonly INSTALL_DIR="/opt/nirj-agent"
...
chmod 0755 "${REPO_DIR}/scripts/run-agent.sh"

if [[ ! -e /etc/nirj-agent/config.yaml ]]; then
    "${VENV_DIR}/bin/nirj-agent" setup \
        --asset-id "${ASSET_ID}" \
        --device-type "${DEVICE_TYPE}"
else
    echo "Preserving existing /etc/nirj-agent/config.yaml"
fi
...
ConditionPathExists=/etc/nirj-agent/config.yaml
...
ExecStart=${REPO_DIR}/scripts/run-agent.sh
Restart=on-failure
RestartSec=30
TimeoutStopSec=90
